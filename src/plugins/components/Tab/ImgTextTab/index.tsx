...
export const ImgTextTab = ({tabs, defaultActive, tabPosition, style: commonStyle}) => {
  ...
  return (
    <div className={style.wrapper} style={transformStyle(commonStyle)}>
      <Tabs activeKey={activeKey} onChange={setActiveKey} tabPosition={tabPosition}>
        {tabs.map((tab, i) => {
          return (
            <Tabs.TabPane key={tab.name} tab={tab.name}>
              {(tab.items || []).map((item, j) => {
                return (
                  <a className={style.itemWrapper} href={item.link} key={`${i}-${j}`}>
